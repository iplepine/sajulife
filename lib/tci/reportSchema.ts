...
export const TCI_REPORT_SCHEMA: Schema = {
  ...
  properties: {
    ...
    sections: {
      type: Type.ARRAY,
      description:
        "영역별 본문을 이 순서로 고정: 기본 성향 / 핵심 조합 캐릭터 / 여덟 결 기질분석 / 반복 실패 루프 / 스트레스 시나리오 / 대인관계 / 직무·진로 / 성장 과제 / 코칭 액션플랜. 기본 성향만 화면에서 바로 펼치고 나머지는 접힌 섹션으로 렌더한다. 값에는 사주·일간·오행·띠·운세를 절대 쓰지 않고, 영문 차원 코드(NS·HA 등)도 노출하지 않는다.",
      items: {
        type: Type.OBJECT,
        properties: {
    ...
    flexibility: {
      type: Type.INTEGER,
      description:
        "유연성 — 상황·관점·계획을 얼마나 잘 바꾸고 적응하는가(외곬·경직의 반대). 7차원 패턴으로 추정한 0~100 정수. 레이더 8번째 축 좌표로 쓰며, '성장 과제' 섹션 본문의 FLEX 근거에서 말한 값과 반드시 일치시킨다.",
    },
    actionPlan: {
      type: Type.ARRAY,
      description:
