...
export const PERSONAL_REPORT_SCHEMA: Schema = {
  ...
  properties: {
    ...
    sections: {
      type: Type.ARRAY,
      description:
        "영역별 본문 9개를 이 순서로 고정: 오행구성 / 기본성향 / 직업운 / 금전운 / 인간관계운 / 건강운 / 대운 / 올해 실행전략 / 주의가 필요한 시기. 오행구성·기본성향은 화면에서 바로 펼치고 나머지는 접힌 섹션으로 렌더한다. '주의가 필요한 시기'는 summary를 \"올 한해는 ~\" 전체 흐름 한 줄로 쓰고, 본문은 주입된 코드 계산값을 자연어로 풀되 한자·명리 용어·겁주기를 쓰지 않으며 이미 지난 달은 부드럽게·앞으로 올 달은 좀 더 단단하게 짚는다.",
      items: {
        type: Type.OBJECT,
        properties: {
