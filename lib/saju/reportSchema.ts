...
export const PERSONAL_REPORT_SCHEMA: Schema = {
  ...
  properties: {
    ...
    sections: {
      type: Type.ARRAY,
      description:
        "영역별 본문 9개를 이 순서로 고정: 오행구성 / 기본성향 / 직업운 / 금전운 / 인간관계운 / 건강운 / 대운 / 올해 실행전략 / 조심할 달. 오행구성·기본성향은 화면에서 바로 펼치고 나머지는 접힌 섹션으로 렌더한다. '조심할 달'은 주입된 코드 계산값을 자연어로 풀고 한자·명리 용어·겁주기를 쓰지 않는다.",
      items: {
        type: Type.OBJECT,
        properties: {
