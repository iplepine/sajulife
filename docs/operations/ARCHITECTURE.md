...
출생 시각을 모르는 경우 시간 pillar는 생략되며 내부 계산 폴백은 prototype 수준이다. 윤달, 분 단위 시주 경계, 표준시/서머타임 보정은 제품화 전 추가 검증이 필요하다.

TODO: 출생시간 보정은 현재 구현하지 않는다. 먼저 `lunar-javascript`가 출생 시각, 지역 표준시, 자시/야자시 경계, 서머타임을 어떻게 처리하는지 확인한 뒤 제품 정책을 정한다.
