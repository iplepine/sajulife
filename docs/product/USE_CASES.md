...
7. 사용자는 가족 리포트 화면에서 가족 상담으로 이어가 구체적인 관계 장면을 질문할 수 있다.

Alternate Flow:
- A. 구성원이 없으면 리포트 생성 버튼을 비활성화한다.
- B. 구성원 정보를 수정/삭제하면 가족 목록과 시각화가 갱신된다.
- C. 구성원 또는 본인 기준 정보가 리포트 생성 이후 바뀌면 기존 리포트가 이전 입력 기준일 수 있음을 안내하고 재생성을 유도한다.
- D. 가족 정보가 바뀐 저장본은 공개 공유 생성 전에 다시 생성하도록 막는다.
