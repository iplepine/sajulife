...
5. 사용자를 홈(`/dashboard`)으로 보낸다.

Alternate Flow:
- A. 이미 세션이 있으면 기존 세션으로 홈 또는 원래 경로에 진입한다.
...
- C. `503 high demand`, `UNAVAILABLE`, `AI 호출 실패` 같은 실패 응답은 리포트로 저장하지 않는다. 과거에 잘못 저장된 실패 저장본은 조회 시 자동 정리하고 미생성 상태로 돌린다.
- D. 사용자가 다시 생성하면 같은 종류의 이전 리포트는 덮어쓴다.
- E. 사용자가 `/saju`의 프로필 수정을 누르면 기존 사주 정보가 채워진 `/onboarding` 폼으로 이동하고, 저장 후 `/saju`로 돌아온다.
...
Defaults / recommendations to show:
- 첫 사용자는 홈에서 현재 고민을 입력하거나, 내 자료(`/materials`)에서 개인 사주/약식 기질 검사 중 하나를 시작하도록 유도한다.
...
- Trigger: 사용자가 내 자료(`/materials`) 또는 `/fusion`에서 융합 리포트를 연다.
...
2. 시스템이 가족 구성원을 `user:{userId}:family`에 저장한다. 저장할 수 있는 가족 수는 제한하지 않는다.
...
- B. 리포트 대상이 4명을 넘으면, 추가 가족 선택은 막고 본인 포함 최대 4명이라는 이유를 보여준다.
- C. 구성원 정보를 수정/삭제하거나 리포트 대상 선택을 바꾸면 가족 목록과 시각화가 갱신된다.
- D. 선택된 구성원 또는 본인 기준 정보가 리포트 생성 이후 바뀌면 기존 리포트가 이전 입력 기준일 수 있음을 안내하고 재생성을 유도한다.
- E. 가족 정보가 바뀐 저장본은 공개 공유 생성 전에 다시 생성하도록 막는다.

Postconditions:
- 가족 구성원 목록, 리포트 대상 선택, 최신 가족 리포트가 저장된다.
...
- 이번 리포트에는 본인을 포함해 최대 4명만 들어가며, 저장한 가족 전체가 자동으로 포함되지는 않는다.

What the system must explain:
- 공유 전 가족 데이터가 공개 스냅샷에 포함될 수 있다는 점.
- 가족 리포트 공유 링크는 로그인 없이 열릴 수 있으므로 필요한 사람에게만 보내야 한다는 점.
- 저장된 가족 리포트가 현재 가족 입력과 다를 수 있으면 다시 생성해야 한다는 점.

Where to reduce choice fatigue:
- 구성원 추가 폼은 기본 1명부터 시작하고, 목록이 있으면 폼을 접어둔다.
- 리포트에 넣을 가족은 가장 궁금한 관계부터 최대 3명만 선택하게 한다.
...
- Trigger: 사용자가 `/consult`에서 질문을 입력하거나, 가족 사주 리포트에서 `가족 상담 시작`을 눌러 상담 요청으로 진입한다.
...
- D. 저장된 리포트가 하나도 없으면 상담 입력 대신 리포트 생성 안내와 내 자료 링크를 보여준다.

Postconditions:
- 상담 기록이 저장되고 기록(`/history`)에서 다시 열 수 있다.
...
- 가족 상담 답변은 실행 순서를 따라 읽을 수 있도록 핵심 진단, 반복 패턴, 말하기 시뮬레이션, 오늘 행동으로 나뉜다는 점.
...
- Trigger: 사용자가 액션 후보를 등록하거나 기록(`/history`)에서 완료 상태를 확인한다. 직접 추가/삭제는 `/coaching` 상세 경로로 유지한다.

Basic Flow:
1. 시스템이 리포트/상담의 액션 후보를 보여준다.
2. 사용자가 일부 또는 전체를 액션 기록에 등록한다.
...
3. 시스템이 만료 설정(기본 30일, 명시적 만료 없음)을 포함한 `share:{token}` 공개 스냅샷을 만든다.
4. 사용자에게 링크 복사, 네이티브 공유, 카카오 공유와 현재 링크의 만료일을 보여준다.
...
- D. 사용자가 링크를 폐기하면 기존 공개 URL과 OG 이미지가 즉시 열리지 않는다.
- E. 사용자가 재발급하면 기존 주소를 폐기하고 새 토큰으로 다시 발급한다.

Postconditions:
- 활성 링크는 같은 토큰으로 최신 스냅샷을 갱신한다. 만료·폐기·재발급 뒤에는 이전 토큰을 되살리지 않는다.
...
- 필요할 때 링크를 즉시 폐기할 수 있고, 기본 만료는 30일이라는 점.
