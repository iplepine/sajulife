...
Basic Flow:
1. 사용자가 가족 이름, 관계, 생년월일, 출생 시각/시각 모름, 달력, 성별을 입력한다.
2. 시스템이 가족 구성원을 `user:{userId}:family`에 저장한다.
3. 시스템이 본인과 구성원의 사주를 계산한다.
4. Gemini가 가족 관계 리포트를 생성한다.
5. 시스템이 `user:{userId}:report:family`에 저장하고 상담 근거 요약을 갱신한다.
...
- Actor: 프로필을 입력한 사용자
- Preconditions: 프로필이 저장되어 있다. 리포트가 있으면 상담 근거 요약이 있거나 백필 가능하다.
- Trigger: 사용자가 `/consult`에서 질문을 입력하고 상담 요청을 누른다.

Basic Flow:
1. 시스템이 질문을 최대 1000자로 검증한다.
2. 시스템이 저장된 리포트 요약을 확인하고, 없거나 낡은 요약은 백필한다.
3. 리포트 요약이 없으면 사주/기질 원본 데이터로 폴백한다.
4. Gemini가 단건 상담 답변과 액션 후보를 생성한다.
5. 시스템이 `user:{userId}:consults` 맨 앞에 저장하고 최근 50개만 유지한다.
6. 사용자는 답변, 근거 라벨, 액션 등록, 히스토리를 본다.
...
What the system must explain:
- 어떤 리포트가 근거로 쓰였는지.
- 리포트가 없을 때는 기본 사주 정보로 답한다는 점.

Where to reduce choice fatigue:
- 근거 리포트 선택 UI를 두지 않고 존재하는 근거를 모두 합쳐 사용한다.

Defaults / recommendations to show:
- 빈 상태에서는 "요즘 가장 마음에 걸리는 일"을 한 문단으로 쓰게 안내한다.
