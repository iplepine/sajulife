...
- 로컬 앱 화면에는 프롬프트 복사/디버그 버튼을 노출하지 않는다.
- 렌더된 프롬프트 확인은 `npm run eval:render` 출력물을 기준으로 한다.
...
1. `npm run eval:render -- saju`를 실행한다.
2. `scripts/eval/out/prompts/`에 생성된 개인 사주 프롬프트 파일을 연다.
3. Gem 새 채팅에 그대로 붙여넣는다.
4. 결과가 현재 프롬프트 규칙을 지키는지 확인한다.
