...
- 모든 사용자 노출 텍스트는 **한국어**. 톤은 **surface별로 다름** (아래).
- ★개인 사주 리포트(`personal-saju`)는 '입 거친 친언니/형' 반말 팩폭 톤★ — 2026-06 전환, [defaults.ts](lib/prompts/defaults.ts) v3:
  - 반말(너/네·~야·~거든). **존댓말·"님" 안 씀.**
  - 한자·명리 용어 **써도 됨 — 단 반드시 옆 괄호에 일상어 풀이**: `식신(食神 - 깊게 파고드는 연구 능력)`. 용어만 툭 던지기 금지.
  - 슬랭·밈(만렙·존버·현타·찐) + 과장 비유 + 시기마다 찰진 '별명' 캐릭터화. 직설 '팩폭'하되 끝은 응원으로 닫음. 비속어는 강조용 1~2회 이하.
  - 강한 확신 단정(100%·무조건·딱) OK — 공포 조장이 아니라 "내가 보증한다"는 든든한 톤.
- ★그 외(앱 UI chrome, 기질·융합·가족 리포트)는 아직 존댓말·"님"·무(無)한자 유지★. 톤 이식은 surface별로 별도 결정.
